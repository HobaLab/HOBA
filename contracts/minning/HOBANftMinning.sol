...
contract HOBANftMinning is ReentrancyGuard, IERC721Receiver, Ownable {
    ...
    // mined 60 days
    uint256 public constant MINED_DAYS = 60;
    ...
    function deposit(uint256 _pid, uint256 _amount, bool _withNft, uint256[] memory _nftIds) public nonReentrant {
        ...
        require(_nftIds.length <= MAX_NFTS, "HOBANftMinning: more than require count");

        updatePool(_pid);

        if (pool.withNft && !user.withNft) {
    ...
    function withdraw(uint256 _pid, uint256 _amount, bool _withNFT) public nonReentrant {
        ...
        if(_amount > 0) {
            if(user.amount == _amount) {
                require(_withNFT, "HOBANftMinning: must withdraw nft");
